...
typedef enum 
{
    UNICOM_MQTT_AUTOREG_REPORT = 1,
    UNICOM_MQTT_AUTOREG_PROPERTY_REPORT,
}unicom_mqtt_queue_msg_t;
...
typedef enum
{
    ...
    UNICOM_MQTT_EVENT_ALL_SUB_SUCCESS,
    UNICOM_MQTT_EVENT_ALL_SUB_FAILED
}unicom_mqtt_event;
...
typedef struct 
{
    ...
    int32_t unsub_msgid;
    bool psm_pub_flag;
    bool shadow_sub_flag;
    bool shadow_pub_flag;
    bool shadow_unsub_flag;
}unicom_mqtt_psm_t;
...
typedef struct
{
    ...
    char *token;                            //token
    int32_t autoreg_period;                    //auto reg timer
 ...
 * @return int32_t 
 */
int32_t unicom_mqtt_sub(int id, const char *topic, int32_t sub_flag, int32_t qos, uint8_t cmd_type, uint16_t* pacid);
 ...
 * @return int32_t 
 */
int32_t unicom_mqtt_bind_sub(int id, int32_t sub_flag, int32_t type, int32_t qos, uint16_t* pacid);
 ...
 * @return int32_t 
 */
int32_t unicom_mqtt_bind_sub_result(void *engine, int id);
 ...
 * @return int32_t 
 */
int32_t unicom_mqtt_shadow_sub(int id, int32_t sub_flag, int32_t type, int32_t qos, uint16_t* pacid);
 ...
 * @return int32_t 
 */
int32_t unicom_mqtt_shadow_sub_result(void *engine, int id);
...
void unicom_mqtt_autoreg_init(void);
 ...
 * @param arg 
 */
void unicom_mqtt_client_destory(void *arg);

/**
 * @brief 根据当前配置判断是否需要动态注册，其次填充自注册信息
 * 
 * @param unimqtt 
 * @return true 
 * @return false 
 */
bool unicom_mqtt_check_need_dynamic(unicom_mqtt_t *unimqtt);
