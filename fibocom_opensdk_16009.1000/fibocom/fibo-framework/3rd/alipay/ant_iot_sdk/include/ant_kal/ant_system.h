...
typedef enum {
    ANT_DEVICE_SN = 0,  		//设备SN，若为opencpu模式，则设备SN与模块SN相同，若是AT模式，则应该返回AT传入的terminal_id
    ANT_DEVICE_MODULE_SN,		//设备的模块SN
    ANT_DEVICE_IMEI,			//设备的IMEI
    ANT_DEVICE_NAME,			//设备产品名称
    ANT_DEVICE_MODEL,			//设备的型号，指的是类别
    ANT_DEVICE_VENDOR,			//设备的厂商名
    ANT_DEVICE_MAC,				//设备的MAC地址，主要是WIFI MAC
    ANT_DEVICE_BT_MAC,			//设备的蓝牙MAC
    ANT_DEVICE_CAMERA_ID,		//设备的摄像头ID
    ANT_DEVICE_ICCID,			//设备通讯卡的ICCID
    ANT_DEVICE_SSID,			//设备所连WIFI的SSID
    ANT_DEVICE_IP,				//设备当前的IP
    ANT_DEVICE_NETMASK,			//设备网络掩码
    ANT_DEVICE_GATEWAY,			//设备网关
    ANT_DEVICE_OS_NAME,			//设备上的OS 操作系统名称
    ANT_DEVICE_OS_VERSION,		//设备上的OS 操作系统版本号
    ANT_DEVICE_CHIP_ARCH,		//设备芯片架构，如"arm"、"mips"等
    ANT_DEVICE_CPU_TYPE,		//设备CPU类型，如
    ANT_DEVICE_MAX
} ant_device_info_e;
 ...
 *         -1： 获取失败
 */
extern ant_s32 ant_system_get_device_info(ant_device_info_e type, char *out, size_t size);
