...
typedef struct{
    bool   onoff; /* KAL_TRUE: enable linger timeout KAL_FALSE: disable linger timeout */
    uint16_t linger_time; /* linger timeout (unit in seconds) */
}ant_soc_linger_struct;
...
typedef struct {
    ant_u8     sin_len;
    ant_u8     sin_family;
    ant_u16    sin_port; //注意字节序，可能需用htons转换
    ant_u32    sin_addr;
    ant_u8     sin_zero[8];
} ant_sockaddr;
...
#define ANT_SOC_FD_ZERO(p)    ant_memset(p, 0x00, sizeof(*(p)))
#define ANT_SOC_FD_SET(s,p)   ((p)->fds_bits[s] = 1)
#define ANT_SOC_FD_CLR(s,p)   ((p)->fds_bits[s] = 0)
#define ANT_SOC_FD_ISSET(s,p) ((p)->fds_bits[s] == 1)
 ...
 *         小于0: 失败，取值参见 ant_soc_errcode_e 定义
 */
 ...
 *
 * @param sock socket句柄
 * @param option 要设置的socket属性，取值参见 ant_soc_option_e 定义
 * 目前option 取值如下之一：
 *     ANT_SOC_LINGER
 *     ANT_SOC_NBIO
 *     ANT_SOC_NODELAY
 * @param val 属性值缓存地址，取值类型范围：
 *     ANT_SOC_LINGER，val指向 ant_soc_linger_struct
 *     ANT_SOC_NBIO，val指向bool
 *     ANT_SOC_NODELAY，val指向bool
 ...
 *
 * @param sock socket句柄
 * @param option 要获取的socket属性，取值参见 ant_soc_option_e 定义
 * @param val 属性值缓存地址
 * @param val_size 属性值缓存大小
 * @return   0： 成功
 *         小于0: 失败，取值参见 ant_soc_errcode_e 定义
 ...
 *            小于0: 发送失败，取值参见 ant_soc_errcode_e 定义
 */
 ...
 * @param data 接收数据缓存地址
 * @param len 接收数据缓存的长度
 * @return 大于等于0: 接收成功，长度为 len
 *              -2:  接收成功，但数据长度比len要少，需要返回 ANT_SOC_WOULDBLOCK
 *         其他小于0: 接收失败，取值参见 ant_soc_errcode_e 定义
 */
 ...
 * @param data 接收数据缓存地址
 * @param len  接收数据的长度
 * @param sock_addr 接收指定的主机地址缓存
 * @param size 传入的sock_addr指向的空间大小，默认调用方传入sizeof(ant_sockaddr)
 * @return 大于等于0: 接收成功，长度为 len
 *              -2:  接收成功，但数据长度比len要少，需要返回 ANT_SOC_WOULDBLOCK
 *         其他小于0: 接收失败，取值参见 ant_soc_errcode_e 定义
 */
 ...
 * @return 大于等于0: 绑定成功
 *            小于0: 绑定失败，取值参见 ant_soc_errcode_e 定义
 */
 ...
 *            小于0: 接收失败，取值参见 ant_soc_errcode_e 定义
 */
 ...
 * @return >0  表示有事件的句柄数。
 *          0  没有事件发生
 *         <0  失败，取值参见 ant_soc_errcode_e 定义
 */
 ...
 *
 * @param sock socket句柄
 * @param error 返回错误码，取值参见 ant_soc_errcode_e 定义
 * @return    0: 成功
 *         小于0: 失败，取值参见 ant_soc_errcode_e 定义
 */
 ...
 *
 * @param sock socket句柄
 * @param error 设置的错误码，取值参见 ant_soc_errcode_e 定义
 * @return    0: 成功
 *         小于0: 失败，取值参见 ant_soc_errcode_e 定义
 */
 ...
 * 将socket错误码转换成字符串
 *
 * @param errcode 错误码，取值参见 ant_soc_errcode_e 定义
 * @return  非空: 返回错误码字符串
 *         NULL: 失败
 */
