 ...
 */
typedef struct {
    ant_u32 message;           // ant message id.     标志消息类型，IOTSDK中固定为19.
    ant_ptr param1;            // user defined value. IOTSDK的消息id在此项中
    ant_ptr param2;            // user defined value. IOTSDK的消息携带的数据结构
...
/**
 * 向指定任务发送事件
 * 注意：消息的发送方为当前调用ant_event_send_msg()的任务。 ant_event_send_msg()也有可能在timer的 callback(callback_on_timer) 中被调用。
 ...
 *         参考 ant_que_sem_status_e
 */
ant_s32 ant_event_send_msg(ant_task_t dst_task_id, ant_msg *msg);
