...
static bool g_network_status = false; // false: not registered true: registration successful
...
static bool g_rrc_status = false; /// false: RRC disconnected true: RRC connected
...
{
    int ret = 0;
    uint8_t ip_type = APP_PDP_IPTYPE_IPV4V6;
    ...
    if (cid == 1) /// If cid is 1, use the default bearer
    {
        fibo_get_ip_info_by_cid(cid, &ip_type, &ip_addr, &ip6_addr);

        if (apn || ip_type != iptype) /// Change the default bearer configuration
        {
            APP_LOG_INFO("set default apn simid=%d apn=%s will cfun0 cfun1", simid, apn);

            /// Set the default bearer configuration
            fibo_initial_pdp_set(1, iptype, apn, auth_proto, apnuser, apnpasswd, simid);

            /// For LTE, it must be re-registered to take effect
