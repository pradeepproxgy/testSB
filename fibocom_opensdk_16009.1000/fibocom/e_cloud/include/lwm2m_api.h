...
uint8_t *ecloud_lwm2m_eventName(uint8_t nEvId);

int lwm2m_is_pending_regcmd(uint8_t ref);

void ecloud_lwm2mPostEvent(uint8_t ref, uint32_t nEventId, uint32_t nResult, uint32_t nParam1, uint32_t nParam2);

int8_t ecloud_lwm2m_new_config_xml(void *file_config, uint16_t size);

int8_t ecloud_lwm2m_new_config(const uint8_t *cmdline);

lwm2m_ret_t ecloud_lwm2m_free_config(uint8_t ref);

lwm2m_ret_t ecloud_lwm2mcfg_set_handler(uint8_t ref, void *taskHandle, osiCallback_t callback, void *userparam);

lwm2m_ret_t ecloud_lwm2mcfg_set_handler_ext(uint8_t ref, void *taskHandle, osiCallback_t callback, void *userparam, uint8_t nDLCI);

lwm2m_config_t *ecloud_lwm2m_get_config(uint8_t configIndex);

uint8_t ecloud_lwm2m_get_ipcSocket(uint8_t configIndex);

void ecloud_lwm2m_parse_buffer(char *buffer, void *value, uint8_t count, void *delim);

lwm2m_ret_t ecloud_lwm2m_excute_cmd(uint8_t *data, uint32_t data_len, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_is_registed(uint8_t ref);

obj_arry_t *ecloud_lwm2m_get_init_objs(uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_add_obj(uint16_t objId, uint32_t instCount, uint8_t *bitmap, uint16_t resCount, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_del_obj(uint16_t objId, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_read_rsp(uint16_t objId, uint16_t instId, uint16_t resId, lwm2m_tpye_t type,
                           const uint8_t *value, uint32_t value_length, bool isEnd, uint8_t result, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_write_rsp(bool result, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_exec_rsp(bool result, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_discover_rsp(uint8_t result, const uint8_t *value, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_notify(uint16_t objId, uint16_t instId, uint16_t resId, const uint8_t *value,
                         uint32_t value_length, lwm2m_tpye_t type, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_update(uint32_t lifetime, bool withObject, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_set_parameter(uint16_t objId, uint16_t instId, uint16_t resId,
                                uint8_t *parameter, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_register(int ref, uint32_t lifetime, uint32_t timeout);

lwm2m_ret_t ecloud_lwm2m_unregister(int ref);

bool ecloud_lwm2m_is_dynamic_ipso(int ref);

lwm2m_ret_t ecloud_lwm2m_set_dynamic_ipso(int ref);

ipso_obj_t *ecloud_lwm2m_get_ipso_obj(uint16_t objId, uint16_t resId, ipso_res_t **ipso_res, uint8_t ref);

uint8_t ecloud_lwm2m_isObjSupport(uint16_t objId, uint8_t ref);

bool ecloud_lwm2m_isResReady(uint16_t objId, uint8_t ref);

void ecloud_lwm2m_registNewObj(uint16_t objId, uint16_t resCount, uint8_t ref);

int ecloud_lwm2m_registNewRes(uint16_t objId, uint8_t *resString, uint8_t ref);

lwm2m_ret_t ecloud_lwm2m_set_send_flag(int ref, int flag);
