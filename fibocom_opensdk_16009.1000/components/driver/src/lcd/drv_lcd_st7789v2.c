...
#ifdef CONFIG_FIBOCOM_LCD_ST7789_240x240
static uint32_t _stReadIdv2_G4(void);
#else
...
#ifndef CONFIG_FIBOCOM_LCD_ST7789_4LINE
...
static uint32_t _stReadIdv2(void)
{
    ...
    #ifndef CONFIG_FIBOCOM_BASE // fibocom modify should not set power level in device driver
...
#else
#ifdef CONFIG_FIBOCOM_LCD_ST7789_240x240
static const lcdOperations_t st7789v2_G4_Operations =
    {
        _stInit,
        _stSleepIn,
        _stEsdCheck,
...
const lcdSpec_t g_lcd_st7789v2_G4 =
    {
        LCD_DRV_ID_ST7789v2,
        LCD_WIDTH,
        LCD_HEIGHT,
        HAL_GOUDA_SPI_LINE_4,
        LCD_CTRL_SPI,
        (lcdOperations_t *)&st7789v2_G4_Operations,
        false,
        0x2a000,
        50000000,
};
static uint32_t _stReadIdv2_G4(void)
{
    ...
    HAL_ERR_T r_err;
    uint8_t id[4] = {0};
    OSI_LOGI(0, "lcd[st7789v2]:  st7789v2_G4   _stReadId \n");
    halPmuSwitchPower(HAL_POWER_LCD, true, true);
    halPmuSetPowerLevel(HAL_POWER_LCD, POWER_LEVEL_2800MV);

    _lcdDelayMs(10);
    st_farmk = g_lcd_st7789v2_G4.is_use_fmark;
    st_line_mode = g_lcd_st7789v2_G4.bus_mode;

    halGoudaReadConfig(st_line_mode, g_lcd_st7789v2_G4.is_use_fmark, g_lcd_st7789v2_G4.fmark_delay);
    _lcdDelayMs(140);
    //hwp_analogReg->pad_spi_lcd_cfg2 |= ANALOG_REG_PAD_SPI_LCD_SIO_SPU;
    _lcdDelayMs(10);
    r_err = halGoudaReadData(0x04, id, 4);
    //hwp_analogReg->pad_spi_lcd_cfg2 =  hwp_analogReg->pad_spi_lcd_cfg2&(~ANALOG_REG_PAD_SPI_LCD_SIO_SPU);
    _lcdDelayMs(10);
    OSI_LOGI(0, "lcd[st7789v2_G4]:  _stReadId LCM: 0x%0x, 0x%x, 0x%0x (read return : %d)\n", id[3], id[2], id[1], r_err);

    ret_id = ((id[3] << 16) | (id[2] << 8) | id[1]);

    if (LCD_DRV_ID_ST7789v2 == ret_id)
    {
        OSI_LOGI(0, "lcd[st7789v2_G4]:  st is using !");
    }
    OSI_LOGI(0, "lcd[st7789v2_G4]: st7789v2_G4 ReadId:0x%x \n",ret_id);
    //halGoudaClose();
    //halPmuSwitchPower(HAL_POWER_LCD, false, false);
    return ret_id;
}
