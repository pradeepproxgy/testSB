...
//#ifdef OSI_LOG_TAG
#define OSI_LOCAL_LOG_TAG OSI_MAKE_LOG_TAG('U', 'S', 'B', 'X')
//#else
...
//#define UX_TRACE_IN_LINE_INSERT(i,a,b,c,d,f,g,h) { OSI_LOGI(0xffffffff,"UX TRACE ERROR, error code : 0x%x",a);}

#define UX_HOST_SIDE_ONLY
#define CONFIG_USB_MUSB_HOST
#define CONFIG_USB_SPRD_DMA
//#define UX_ENFORCE_SAFE_ALIGNMENT
#define UX_REGULAR_MEMORY_DYNAMIC
...
#define UX_MS_TO_TICK(ms) osiMsToOSTick(ms) // ((ms) * (UX_PERIODIC_RATE) / 1000)
...
#define UX_HOST_HCD_THREAD_STACK_SIZE (6 * 1024)
#endif
...
#define UX_INTERRUPT_SAVE_AREA //taskDECLARE_CRITICAL(); //TX_INTERRUPT_SAVE_AREA
#endif
...
#define UX_DISABLE                     \
    ...
    } //taskENTER_CRITICAL(); // TX_DISABLE
#endif

#ifndef UX_RESTORE
#define UX_RESTORE osiExitCritical(critical); //taskEXIT_CRITICAL();//TX_RESTORE
#endif
...
#ifndef UX_TIMER
#define UX_TIMER osiTimer_t
#endif

#ifndef UX_THREAD
#define UX_THREAD osiThread_t
#endif

#ifndef UX_CALLBACK
#define UX_CALLBACK osiCallback_t
#endif

#ifndef UX_AUTO_START
#define UX_AUTO_START 0 // TX_AUTO_START
#endif
...
#define UX_AUTO_ACTIVATE 1
#endif

#ifndef UX_NO_ACTIVATE
#define UX_NO_ACTIVATE 0
#endif

#ifndef UX_THREAD_GET_SYSTEM_STATE
#define UX_THREAD_GET_SYSTEM_STATE() IS_IRQ() //TX_THREAD_GET_SYSTEM_STATE
#endif

#ifndef UX_WAIT_FOREVER
#define UX_WAIT_FOREVER OSI_WAIT_FOREVER //TX_WAIT_FOREVER
#endif

#ifndef UX_NO_WAIT
#define UX_NO_WAIT 0 //TX_NO_WAIT
#endif
...
#ifndef UX_SEMAPHORE
#define UX_SEMAPHORE osiSemaphore_t
#endif

/* Define mutex related things.  */

#ifndef UX_MUTEX
#define UX_MUTEX osiMutex_t
#endif

/* Define event flags group related things.  */

#ifndef UX_EVENT_FLAGS_GROUP
#define UX_EVENT_FLAGS_GROUP EventGroupHandle_t
#endif

#ifndef UX_OR_CLEAR
#define UX_OR_CLEAR 1 //TX_OR_CLEAR
#endif

#ifndef UX_OR
#define UX_OR 0 //TX_OR
#endif

#ifndef UX_AND_CLEAR
#define UX_AND_CLEAR 3 //TX_AND_CLEAR
#endif

#ifndef UX_AND
#define UX_AND 2 // TX_AND
#endif
...
#define UX_SAFE_ALIGN UX_ALIGN_32
...
#define UX_CONTROL_TRANSFER_TIMEOUT 3000
#define UX_NON_CONTROL_TRANSFER_TIMEOUT 5000
#define UX_PORT_ENABLE_WAIT 50
#define UX_DEVICE_ADDRESS_SET_WAIT 500
...
#define UX_HOST_HCD_MUSB_ERROR 0x64
#define UX_HSOTG_IS_NULL 0X65
...
#define UX_MSG_ERROR 0x91
#define UX_CALLBACK_ERROR 0x92
...
#define UX_HCD_TEST_MODE 19

typedef enum
{
    MUSB_IDLE = 0,
    MUSB_RECOVER_FROM_BABBLE = 1,
    MUSB_START_WORK,
    MUSB_EP_INT,
    MUSB_DMA_INT,
    MUSB_SUSPEND,
    MUSB_RESUME,
    MUSB_TEST_MODE,
    MUSB_TRANSFER_CALLBACK,
    MUSB_DELAYED_RECOVER_WORK,
    MUSB_DELAYED_FINISH_RESUME_WORK,
    MUSB_DELAYED_RESET_WORK,
} MSG_TYPE;
...
#define UX_RH_ENUMERATION_RETRY 1
...
typedef struct
{
    ...
    struct list_head ux_application_list;
    UINT(*app_start)
    (void *);
    UINT(*app_stop)
    (void *);
...
#define UX_DEVICE_PARENT_SET(d, p)   \
    do                               \
    {                                \
        (d)->ux_device_parent = (p); \
    } while (0)
...
#define UX_DEVICE_MAX_POWER_SET(d, p)   \
    do                                  \
    {                                   \
        (d)->ux_device_max_power = (p); \
    } while (0)
#else
...
#define UX_DEVICE_PORT_LOCATION_SET(d, l)   \
    do                                      \
    {                                       \
        (d)->ux_device_port_location = (l); \
    } while (0)
...
typedef struct UX_MSG_STRUCT
{
    //UX_THREAD *thread_id;
    //UX_CALLBACK callback;
    MSG_TYPE msg_type;
    void *ctx;
} UX_MSG;
...
typedef struct UX_HCD_STRUCT
{
    ...
    ULONG ux_hcd_io;
    UINT(*ux_hcd_entry_function)
    (struct UX_HCD_STRUCT *, UINT, VOID *);
    //UX_MSG        ux_msg;
...
typedef struct UX_SYSTEM_STRUCT
{
    ...
    VOID(*ux_system_error_callback_function)
    (UINT system_level, UINT system_context, UINT error_code);
#endif
    ...
    VOID(*ux_system_debug_callback_function)
    (UCHAR *debug_message, ULONG debug_value);
#endif
} UX_SYSTEM;
...
#if UX_MAX_DEVICES > 1
    VOID(*ux_system_host_enum_hub_function)
    (VOID);
#endif
...
#define UX_SYSTEM_HOST_MAX_CLASS_SET(n)                  \
    do                                                   \
    {                                                    \
        _ux_system_host->ux_system_host_max_class = (n); \
    } while (0)
#else
...
#define UX_SYSTEM_HOST_MAX_HCD_SET(n)                  \
    do                                                 \
    {                                                  \
        _ux_system_host->ux_system_host_max_hcd = (n); \
    } while (0)
#else
...
#if UX_MAX_DEVICES > 1
#define UX_SYSTEM_HOST_MAX_DEVICES_SET(n)                  \
    do                                                     \
    {                                                      \
        _ux_system_host->ux_system_host_max_devices = (n); \
    } while (0)
...
typedef struct UX_SYSTEM_SLAVE_STRUCT
{
    ...
    UINT(*ux_system_slave_change_function)
    (ULONG);
    ULONG ux_system_slave_device_vendor_request;
    UINT(*ux_system_slave_device_vendor_request_function)
    (ULONG, ULONG, ULONG, ULONG, UCHAR *, ULONG *);

} UX_SYSTEM_SLAVE;

#if UX_MAX_SLAVE_CLASS_DRIVER > 1
#define UX_SYSTEM_DEVICE_MAX_CLASS_SET(n)                  \
    do                                                     \
    {                                                      \
        _ux_system_slave->ux_system_slave_max_class = (n); \
    } while (0)
...
typedef struct UX_SYSTEM_OTG_STRUCT
{
    ...
    VOID(*ux_system_otg_vbus_function)
    (ULONG);
    VOID(*ux_system_otg_change_mode_callback)
    (ULONG);
} UX_SYSTEM_OTG;
...
#define ux_host_stack_application_register _ux_host_stack_application_register
#define ux_host_stack_application_call _ux_host_stack_application_call
...
UINT ux_is_cache_safe_address(void *memory);
