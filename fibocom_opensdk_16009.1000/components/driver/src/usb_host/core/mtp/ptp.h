...
#if 0
#define MTP_LOGE(fmt, ...) OSI_LOGE(0xffffffff, "MTP: " fmt, ##__VA_ARGS__)
#define MTP_LOGW(fmt, ...) OSI_LOGW(0xffffffff, "MTP: " fmt, ##__VA_ARGS__)
#define MTP_LOGI(fmt, ...) OSI_LOGI(0xffffffff, "MTP: " fmt, ##__VA_ARGS__)
#define MTP_LOGD(fmt, ...) OSI_LOGI(0xffffffff, "MTP: " fmt, ##__VA_ARGS__)
#define MTP_LOGV(fmt, ...) OSI_LOGV(0xffffffff, "MTP: " fmt, ##__VA_ARGS__)

#define MTP_DEV_LOGI(fmt, ...) OSI_LOGI(0xffffffff, "MTP DEVICE INFO: " fmt, ##__VA_ARGS__)

#define MTP_LOGXE(fmt, ...) OSI_LOGXE(OSI_LOGPAR_S, 0, "MTP: " fmt, ##__VA_ARGS__)
#define MTP_LOGXW(fmt, ...) OSI_LOGXW(OSI_LOGPAR_S, 0, "MTP: " fmt, ##__VA_ARGS__)
#define MTP_LOGXI(fmt, ...) OSI_LOGXI(OSI_LOGPAR_S, 0, "MTP: " fmt, ##__VA_ARGS__)
#define MTP_LOGXD(fmt, ...) OSI_LOGXI(OSI_LOGPAR_S, 0, "MTP: " fmt, ##__VA_ARGS__)
#define MTP_LOGXV(fmt, ...) OSI_LOGXV(OSI_LOGPAR_S, 0, "MTP: " fmt, ##__VA_ARGS__)

#define MTP_DEV_LOGXI(fmt, ...) OSI_LOGXI(OSI_LOGPAR_S, 0, "MTP DEVICE INFO: " fmt, ##__VA_ARGS__)
#endif
...
UINT ptp_getdeviceinfo(PTPParams *params, PTPDeviceInfo *deviceinfo);

uint16_t ptp_generic_no_data(PTPParams *params, uint16_t opcode, unsigned int cnt, ...);

UINT ptp_opensession(PTPParams *params);
...
UINT ptp_getstorageids(PTPParams *params, PTPStorageIDs *storageids);
UINT ptp_getstorageinfo(PTPParams *params, uint32_t storageid,
                        UX_HOST_CLASS_PIMA_STORAGE *storageinfo);
/**
 * ptp_formatstore:
 * params:      PTPParams*
 *              storageid               - StorageID
...
uint16_t ptp_getobject(PTPParams *params, uint32_t handle,
                       unsigned char **object, uint32_t *data_len);
...
UINT ptp_getdevicepropdesc(PTPParams *params, uint16_t propcode,
...
UINT ptp_mtp_getobjectpropdesc(PTPParams *params, uint16_t opc, uint16_t ofc,
...
PTPObject *ptp_add_object_to_cache(PTPParams *params, uint32_t handle);
...
int ptp_unpack_DPD(PTPParams *params, unsigned char *data, PTPDevicePropDesc *dpd, unsigned int dpdlen);
time_t ptp_unpack_PTPTIME(const char *str);
int ptp_unpack_OPD(PTPParams *params, unsigned char *data, PTPObjectPropDesc *opd, unsigned int opdlen);

int ptp_unpack_DPV(
    PTPParams *params, unsigned char *data, int *offset, int total,
    PTPPropertyValue *value, uint16_t datatype);
void ptp_unpack_DI(PTPParams *params, unsigned char *data, PTPDeviceInfo *di, unsigned int datalen);
int ptp_unpack_OPL(PTPParams *params, unsigned char *data, MTPProperties **pprops, unsigned int len);
uint32_t ptp_unpack_uint16_t_array(PTPParams *params, unsigned char *data, uint16_t offset, uint16_t **array);
uint32_t ptp_pack_DPV(PTPParams *params, PTPPropertyValue *value, unsigned char **dpvptr, uint16_t datatype);
uint32_t ptp_pack_OI(PTPParams *params, PTPObjectInfo *oi, unsigned char **oidataptr);
void ptp_unpack_OI(PTPParams *params, unsigned char *data, PTPObjectInfo *oi, unsigned int len);
void ptp_unpack_OH(PTPParams *params, unsigned char *data, PTPObjectHandles *oh, unsigned int len);
uint32_t
...
uint32_t ptp_pack_OPL(PTPParams *params, MTPProperties *props, int nrofprops, unsigned char **opldataptr);

#define ptp_debug(params, fmt, ...) OSI_LOGI(0xffffffff, "ptp: " fmt, ##__VA_ARGS__)
#define ptp_error(params, fmt, ...) OSI_LOGE(0xffffffff, "ptp: " fmt, ##__VA_ARGS__)
