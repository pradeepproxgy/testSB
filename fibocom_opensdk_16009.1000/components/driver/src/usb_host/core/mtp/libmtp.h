...
LIBMTP_error_number_t LIBMTP_Detect_Raw_Devices(LIBMTP_raw_device_t **devices,
                                                int *numdevs, UX_HOST_CLASS_PIMA *pima);
...
//LIBMTP_error_number_t LIBMTP_Get_Connected_Devices(LIBMTP_mtpdevice_t **device_list, UX_HOST_CLASS_PIMA *pima);
LIBMTP_error_number_t LIBMTP_Get_Connected_Devices(UX_HOST_CLASS_PIMA *pima);
...
void add_object_to_cache(LIBMTP_mtpdevice_t *device, uint32_t object_id);
int LIBMTP_Delete_File(LIBMTP_mtpdevice_t *device, LIBMTP_devicestorage_t *storage, LIBMTP_file_t *file);
 ...
 * @{
 */
void LIBMTP_Read_Event(UX_HOST_CLASS_PIMA_EVENT *event);
