...
#define FX_PROTECT                                        \
    {                                                     \
        ...
        ux_utility_mutex_on(media_ptr->fx_media_protect); \
    }
#define FX_UNPROTECT ux_utility_mutex_off(media_ptr->fx_media_protect);
#endif
...
#define FX_CALLER_CHECKING_EXTERNS extern volatile ULONG _tx_thread_system_state;

#define FX_CALLER_CHECKING_CODE              \
    if ((UX_THREAD_GET_SYSTEM_STATE()) ||    \
        (_tx_thread_current_ptr == UX_NULL)) \
        return (FX_CALLER_ERROR);

#else
#define FX_CALLER_CHECKING_EXTERNS            \
    extern UX_THREAD *_tx_thread_current_ptr; \
