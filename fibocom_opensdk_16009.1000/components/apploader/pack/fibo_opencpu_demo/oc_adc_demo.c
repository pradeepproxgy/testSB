...
static void adc_thread_entry(void *param)
{
    ...
    ret = fibo_hal_adc_get_data(channel, scale);
    fibo_textTrace("test fibo_hal_adc_get_data scale %d, data=%d", scale, ret);
    fibo_taskSleep(1000);

    fibo_textTrace("test fibo_hal_adc_get_data_polling channel %d", channel);
    fibo_hal_adc_get_data_polling(channel, &data);
    fibo_textTrace("test fibo_hal_adc_get_data_polling channel 0, data=%d", data);
    fibo_taskSleep(1000);
