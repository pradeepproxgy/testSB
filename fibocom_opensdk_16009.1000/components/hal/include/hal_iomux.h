...
/**
 * \brief set pad pull up/down/none
 *
 * When the pull up/down doesn't match pad properties, it will be
 * regarded as an error, and return false.
 *
 * \param key_reg register address
 * \param type pull up/down/none type
 * \return
 *      - true on sucess
 *      - false on fail, invalid parameter
 */
bool halIomuxSetPadProp(uint32_t *key_reg,halIomuxPullType_t type);
#endif
