...
typedef struct
{
    ...
    */
    ...
    uint8_t Rac;
    uint8_t CellId[2];
//modif by MTC0741-312(SPCSS01344709) for diff LOCAL_RELEASE RRC and other RRC,start
#ifdef CONFIG_FIBOCOM_DIDI
    uint8_t releasetype;
    /*
    0 ERRC_NORMAL_RELEASE
    1 LTE_LOCAL_RELEASE
    0xFF default
    */
#endif
//modif by MTC0741-312(SPCSS01344709) for diff LOCAL_RELEASE RRC and other RRC,end
} CFW_NW_CONN_STATUS_INFO;

#define CFW_BLACK_WHITE_PLMN_NUM 30    //merge patch SPCSS01345734
typedef struct
{
...
#ifdef CONFIG_FIBOCOM_BASE //merge patch SPCSS01326898
typedef struct
{
    uint8_t cdrxConfiged;
    uint16_t pci;
    uint32_t freq;
    uint8_t reserved;
} CFW_CDRX_RPT_IND_T;

uint32_t CFW_NwSetCdrxRptCfg(uint8_t bEnable, CFW_SIM_ID nSimID);
#endif
#endif //if cfw.h
