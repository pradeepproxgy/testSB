...
{
    osiTimer_t *Wakeup_timer = NULL;
    Wakeup_timer = osiTimerCreate(netThreadID, cb, NULL);
    return Wakeup_timer;
}
bool sys_StartWakeupTimer(void *timer, uint32_t ms)
{
    osiTimer_t *Wakeup_timer = (osiTimer_t *)timer;
    if (Wakeup_timer == NULL)
        return false;
    osiTimerStop(Wakeup_timer);
    osiTimerStart(Wakeup_timer, ms);
    return true;
}
