...
static char *genRegData(int simid)
{
    ...
    if (regdatajs == NULL)
    {
        OSI_LOGI(0x10007509, "JsonString error...");
        cJSON_Delete(pJsonRoot);
        return NULL;
    }
    else
        OSI_LOGXI(OSI_LOGPAR_S, 0x1000750a, "genRegData :%s\n", regdatajs);
...
static void parseJson(char *pMsg, reg_ctrl_t *reg_ctrl)
{
    if (NULL == pMsg)
    {
        OSI_LOGI(0x10007504, "parseJson: pMsg is NULL!!!\n");
        return;
    }
    cJSON *pJson = cJSON_Parse(pMsg);
    if (NULL == pJson)
    {
        OSI_LOGI(0x10007505, "parseJson: pJson is NULL!!!\n");
        return;
    }
    char *regdatajs = cJSON_PrintUnformatted(pJson);
    OSI_LOGXI(OSI_LOGPAR_S, 0x10007506, "parseJson: %s\n", regdatajs);

    cJSON *pSub = cJSON_GetObjectItem(pJson, "resultCode");
    if (NULL != pSub)
    {
        OSI_LOGXI(OSI_LOGPAR_S, 0x10007507, "resultCode: %s\n", pSub->valuestring);
    ...
    if (NULL != pSub)
    {
        OSI_LOGXI(OSI_LOGPAR_S, 0x10007508, "resultDesc: %s\n", pSub->valuestring);
        strncpy(reg_ctrl->resultDesc, pSub->valuestring, 10);
    }

    if (gRegCtrl.cb != NULL)
    {
        OSI_LOGI(0x1000752e, "notify to uplower\n");
...
static void coap_user_cb(coap_client_t *client, coap_method_t method, bool result)
{
    OSI_LOGI(0x1000752f, "coap_user_cb (%d):%d", method, result);
...
static void do_regProcess(void *param)
{
    ...
    if (NULL == regdata)
    {
        OSI_LOGI(0x1000751b, "regdata error...");
        coap_client_free(client);
        goto retry;
    }
    gRegCtrl.status = coap_client_getStatus(client);
