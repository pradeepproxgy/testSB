...
static u32_t output_cb(ppp_pcb *pcb, u8_t *data, u32_t len, void *ctx)
{
    ...
    //OSI_LOGI(0xffffffff, "output_cb uart_SendData ret =%d\n", len);
    return len;
}
...
void AT_PppTerminateInd(atCmdEngine_t *engine)
{
    ...
    //OSI_LOGI(0xffffffff, "AT_PppTerminateInd nDLCI = %d,isPppActived=%d", PARA->engine->channel_id, isPppActived);
    isPppActived = 0;
...
static void ppp_notify_phase_cb(ppp_pcb *pcb, u8_t phase, void *ctx)
{
    ...
    ctx_cb_t *ctx_p = (ctx_cb_t *)ctx;
    //OSI_LOGI(0xffffffff, "ppp_notify_phase_cb pcb=0x%x phase=%d,gprs status:%d", pcb, phase, ctx_p->gprsState);
...
void AT_PppProcess(atCommand_t *PARA, uint8_t nSim, uint8_t nCid)
{
    ...
    //OSI_LOGI(0xffffffff, "AT_PppProcess:%d,%d,%d,isPppActived=%d", nDLCI, nSim, nCid, isPppActived);
    atDispatch_t *d = atCmdGetDispatch(PARA->engine);
