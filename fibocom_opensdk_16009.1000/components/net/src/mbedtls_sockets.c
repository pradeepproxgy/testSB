...
#if defined(CONFIG_MBEDTLS_TCPIP_SUPPORT)
...
static const char *pers = "rda_sock";
...
static uint32_t g_tlsInterRxTime = 300;
...
static int32_t MbedtlsSocket_dataleft(TLSSOCK_HANDLE handle);
...
static void Mbedsocket_InternalCloseInd(uint8_t ssid);
...
static void MBED_InternalRxTimerTimeout(void *param)
{
    ...
    sys_arch_printf("MBED_InternalRxTimerTimeout,ssid:%d", (int8_t)ssid);
...
static void MBEDTLS_TcpipRsp(void *param)
{
    ...
    case EV_CFW_TCPIP_SOCKET_CONNECT_RSP:
        if (tlssock_ptr->server_fd.fd == uSocket)
        {
            ...
            if (ret == 0 && (-1 != ssid))
            {
                ...
                MBEDSOCKET_TRACE("MBEDTLS_SOCKET: Connect Success, start InternalRxTimer :%ld ms", g_tlsInterRxTime);
    ...
    case EV_CFW_TCPIP_BEARER_LOSING_IND:
        if (tlssock_ptr->server_fd.fd == uSocket)
        {
            ...
            //if (tlssock_ptr->server_fd.fd != INVALID_SOCKET)//do not close socket in tcp event , let up-layer do socket close
            //{
            ...
            if (tlssock_ptr->state == TLSSOCK_STATE_CONNECTED)
            {
                ...
                if (-1 != ssid)
                {
                    if (1 == MbedtlsSocket_dataleft((TLSSOCK_HANDLE)tlssock_ptr))
                    {
                        tlssock_ptr->state = TLSSOCK_STATE_REFUSED;
                    }
                    else
                    {
                        Mbedsocket_InternalCloseInd(ssid);
                    }
                }
            }
...
static bool socket_connect(TLSSOCK_HANDLE handle, uint32_t ip, uint16_t port)
{
    ...
    if (ctx->fd == INVALID_SOCKET)
    {
        sys_arch_printf("socket_connect: Create socket[%d] fail", ctx->fd);
        return false;
    }
    tlssock_ptr->sock_id = ctx->fd;
    sys_arch_printf("socket_connect: Create socket nCid[%d] nSimid[%d]", nCid, nSimid);
    struct netif *netif_bind = NULL;

    if (nCid == TLSSOCK_INVALID_CID)
    {
        sys_arch_printf("socket_connect: TLSSOCK_INVALID_CID");
    ...
    if (netif_bind == NULL)
    {
        sys_arch_printf("socket_connect: netif_bind = NULL");
        return FALSE;
    }
    ...
    if (0 != result)
    {
        ...
        sys_arch_printf("socket_connect: CFW_TcpipSocketBind result[%d-%ld]", result, sock_error_code);
        return FALSE;
    }
    ...
    {
        sys_arch_printf("Socket[%d] connect fail", ctx->fd);
        CFW_TcpipSocketClose(ctx->fd);
        ctx->fd = INVALID_SOCKET;
        return FALSE;
    }
...
static void Mbedsocket_CloseEvent(TLSSOCK_HANDLE handle)
{
    MBEDSOCKET_TRACE("Mbedsocket_CloseEvent enter");
    TLSSOCK_CONTEXT *tlssock_ptr = (TLSSOCK_CONTEXT *)handle;
    if (tlssock_ptr == NULL)
    {
...
static void Mbedsocket_InternalCloseInd(uint8_t ssid)
{
    ...
    ev.id = EV_CFW_TLSSOCK_INTERNAL_CLOSE_IND;
    ev.param1 = ssid;
...
static void Mbedsocket_TaskEntry(void *pData)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            case EV_CFW_TLSSOCK_INTERNAL_CLOSE_IND:
                ssid = (uint8_t)event.param1;
                if (g_tlssock_context[ssid] != NULL && g_tlssock_context[ssid]->is_used)
                {
                    Mbedsocket_CloseEvent((TLSSOCK_HANDLE)(g_tlssock_context[ssid]));
                }
                break;
...
int32_t MbedtlsSocket_dataleft(TLSSOCK_HANDLE handle)
{
    TLSSOCK_CONTEXT *tlssock_ptr = (TLSSOCK_CONTEXT *)handle;
    struct data_buf_t *rx_decrypt_buf;
    rx_decrypt_buf = &(tlssock_ptr->rx_buf);
    osiSemaphoreAcquire(s_read_sema);
    uint32_t data_left = data_buf_len(rx_decrypt_buf);
    MBEDSOCKET_TRACE("tls_Recv,data_left=%ld", data_left);
    osiSemaphoreRelease(s_read_sema);
    int32_t avail_size = mbedtls_ssl_get_bytes_avail(&(tlssock_ptr->ssl));
    MBEDSOCKET_TRACE(" %ld bytes left in tls", avail_size);
    if ((avail_size == 0) && (data_left == 0))
    {
        return 0;
    }
...
int32_t MbedtlsSocket_Recv(TLSSOCK_HANDLE handle, uint8_t *buf, uint32_t len)
{
    ...
    if (tlssock_ptr->state >= TLSSOCK_STATE_CONNECTED)
    {
        ...
        tlssock_ptr->rx_data_len = data_buf_len(rx_decrypt_buf); //AT_RingBuf_DataSize
        osiSemaphoreRelease(s_read_sema);
        if (0 == MbedtlsSocket_dataleft((TLSSOCK_HANDLE)tlssock_ptr) && TLSSOCK_STATE_REFUSED == tlssock_ptr->state)
        {
            Mbedsocket_CloseEvent(handle);
        }
    }
