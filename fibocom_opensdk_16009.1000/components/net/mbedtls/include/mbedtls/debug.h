...
#define MBEDTLS_SSL_DEBUG_MSG( level, args )     do { if((level) <= 5){OSI_LOGI args;}} while(0)
#define MBEDTLS_SSL_DEBUG_MSGS( level, args )     do { if((level) <= 5){OSI_LOGXI args;}} while(0)
#define MBEDTLS_SSL_DEBUG_RET( level, traceid, text, ret )  do { if((level) <= 5 ){OSI_LOGI (traceid,text,ret);}} while(0)
...
#define MBEDTLS_SSL_DEBUG_MSGS( level, args )                   \
    mbedtls_debug_print_msg( ssl, level, __FILE__, __LINE__,    \
                             MBEDTLS_DEBUG_STRIP_PARENS args )

#define MBEDTLS_SSL_DEBUG_RET( level, traceid, text, ret )                \
...
#define MBEDTLS_SSL_DEBUG_MSGS( level, args )            do { } while( 0 )
#define MBEDTLS_SSL_DEBUG_RET( level, traceid, text, ret )       do { } while( 0 )
